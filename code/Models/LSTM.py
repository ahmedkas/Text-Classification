...
def Train(x, y, epochs = 100, batch_size = 128, LR = 0.001, n_layers = 3, layer_size = 128, dropout = False, MaxPooling = False, Embedding = False, vocab_size = None, embedding_dim = 64, loss = "binary_crossentropy",train=False):
    ...
    model.add(keras.layers.LSTM(128,unroll=True,dropout=0.3,return_sequences=True))


    for i in range(n_layers):
        if i != n_layers-1:
            model.add(keras.layers.LSTM(64,dropout=0.3,unroll=True,return_sequences=True))
        else :
            model.add(keras.layers.LSTM(64, dropout=0.3, unroll=False))
